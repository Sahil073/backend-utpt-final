...
// ─── Enrich leaderboard entries with full user + stat data ──
const enrichLeaderboard = async (
  ...
  const [usersRes, codingRes, githubRes, scoreRes] = await Promise.all([
    supabase
      .from("users")
  ...
  const scoreMap  = new Map((scoreRes.data   || []).map((s) => [s.user_id, s]));
  ...
  return entries.map((e) => ({
    ...
    code_commits:       githubMap.get(e.userId)?.code_commits    ?? null,
...
// ─── Read a paginated slice from a Redis sorted set ──────────
const getLeaderboardFromRedis = async (
  key:   string,
  page:  number,
  limit: number
) => {
  ...
  for (let i = 0; i < raw.length; i += 2) {
    entries.push({
      userId: raw[i],
      score:  Math.round(parseFloat(raw[i + 1])), // Redis stores scores as floats
...
// ─── Ensure leaderboard is populated (lazy recompute) ────────
const ensureLeaderboard = async (key: string): Promise<void> => {
...
export const getGlobalLeaderboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await ensureLeaderboard("leaderboard:global");
...
// ────────────────────────────────────────────────────────────
// GET /leaderboard/batch/:batch?page=1
// ────────────────────────────────────────────────────────────
export const getBatchLeaderboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      meta: { total, page, limit, pages: Math.ceil(total / limit), batch },
      message: `Batch ${batch} leaderboard fetched`,
    });
  } catch (err) {
...
// ────────────────────────────────────────────────────────────
// GET /leaderboard/specialization/:spec?page=1
// ────────────────────────────────────────────────────────────
export const getSpecLeaderboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      meta: { total, page, limit, pages: Math.ceil(total / limit), specialization: spec },
      message: `${spec} leaderboard fetched`,
    });
  } catch (err) {
...
// Queries Postgres directly (bypasses Redis) for flexible filtering
// ────────────────────────────────────────────────────────────
export const getFilteredLeaderboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Map public metric names to DB column names
    ...
    // Filter students — only active portal students appear
    ...
    const { data: matchedUsers, error: userErr } = await userQuery;

    if (userErr) {
      res.status(500).json({ success: false, data: null, message: userErr.message });
      return;
    }
    ...
    if (!matchedUsers || matchedUsers.length === 0) {
      res.status(200).json({
        success: true, data: [],
        meta: { metric, batch: batch || null, specialization: specialization || null, page: pageNum, limit, total: 0 },
        message: "No students match the filter",
      });
      return;
    }
    ...
    // Fetch scores ordered by the selected metric
    const { data: scores, error: scoreErr, count } = await supabase
      .from("scores")
      .select("user_id, total_score, academics_score, coding_score, dev_score", { count: "exact" })
      .in("user_id", userIds)
      .order(scoreCol, { ascending: false })
      .range(offset, offset + limit - 1);

    if (scoreErr) {
      res.status(500).json({ success: false, data: null, message: scoreErr.message });
      return;
    }
    ...
    if (scoreIds.length === 0) {
      res.status(200).json({
        success: true, data: [],
        meta: { metric, batch: batch || null, specialization: specialization || null, page: pageNum, limit, total: count ?? 0 },
    ...
    const [usersRes, codingRes, githubRes] = await Promise.all([
      ...
      supabase.from("github_stats").select("user_id, total_commits, code_commits").in("user_id", scoreIds),
    ]);
    ...
    const data = (scores || []).map((s, idx) => ({
      ...
      metric_score:       (s as Record<string, any>)[scoreCol] ?? 0,
      ...
      code_commits:       githubMap.get(s.user_id)?.code_commits    ?? null,
    }));
  ...
  try {
    ...
    if (rankRaw === null) {
      res.status(200).json({
        ...
        message: "Not ranked yet — your stats may still be syncing.",
      });
      return;
    }

    const myRank      = rankRaw + 1;
    const myScoreRaw  = await redis.zscore("leaderboard:global", userId);
    const myScore     = Math.round(parseFloat(myScoreRaw || "0"));

    // Fetch 5 above and 5 below for context
    ...
    for (let i = 0; i < raw.length; i += 2) {
      neighbourEntries.push({
        userId: raw[i],
        score:  Math.round(parseFloat(raw[i + 1])),
        rank:   start + Math.floor(i / 2) + 1,
      });
    }

    const neighbors = await enrichLeaderboard(neighbourEntries);

    res.status(200).json({
      success: true,
      data:    { rank: myRank, score: myScore, neighbors },
