...
exports.syncUserGitHub = void 0;
const axios_1 = __importDefault(require("axios"));
const env_1 = require("../config/env");
const db_1 = require("../config/db");
const CommitLog_model_1 = require("../models/CommitLog.model");
...
const getCommitsFromRepo = async (username, repo) => {
    try {
        const since = new Date();
        since.setDate(since.getDate() - 30); // last 30 days
        const { data } = await githubAPI.get(`/repos/${username}/${repo}/commits`, {
            params: {
                author: username,
...
// ─── Main sync function ──────────────────────────────────────
const syncUserGitHub = async (userId, githubUsername) => {
    ...
    try {
        ...
        await CommitLog_model_1.CommitLog.create({
            user_id: userId,
            synced_at: new Date(),
            commits: allCommits,
        });
        // Update github_stats in Postgres (upsert)
        await db_1.supabase.from("github_stats").upsert({
            user_id: userId,
        ...
        console.log(`✅ GitHub sync complete for ${githubUsername}: ${codeCommits} code, ${trivialCommits} trivial`);
    }
    catch (err) {
...
exports.syncUserGitHub = syncUserGitHub;
